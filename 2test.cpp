  ...
  for (int i = 1; i <= rows; i++) {
    ...
    if (condType != GLP_UP) {
      if (condType == GLP_LO) {
        ...
        int bound = glp_get_row_lb(prob, i);
        // The 0 is ignored in the GLP_UP case
        glp_set_row_bnds(prob, i, GLP_UP, 0, -1 * bound);
      ...
      if (condType == GLP_DB) {
        ...
        glp_set_row_bnds(prob, newRow, GLP_UP, 0, bound);
...
int main(int argc, char **argv) {
  ...
  // const char *filename = "TSP.mps";
  const char *filename = "../../test.lp";

  // if(glp_read_mps(prob, GLP_MPS_FILE, NULL, filename)) {
  if (glp_read_lp(prob, NULL, filename)) {
    return -1;
  } else {
